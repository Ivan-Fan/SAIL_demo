...
highv: high value
'''
def graph_ranks_original(avranks, names, p_values, cd=None, cdmethod=None, lowv=None, highv=None,
...
# Form cliques
def form_cliques_original(p_values, nnames):
    m = len(nnames)
...
def draw_cd_diagram(df_perf=None, alpha=0.05, name='cd-diagram.pdf', height=3, width=9, **kwargs):
    ...
    if p_values is None:
        ...
        '''
        print("Avg Rank: ", average_ranks.values)
        fig = graph_ranks(average_ranks.values, average_ranks.keys(), p_values,
                    cd=None, reverse=True, height=height, width=width, **kwargs)
