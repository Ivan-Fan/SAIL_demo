...
- Clustering: A review of the clustering performance of equal-sized representations for each symbolic reprsentation using an equivalent distance metric for all methods. Presents average Rand Index (RI), pairwise, and ranked statistical comparisons.
...
- Anomaly Detection: A case study of the anomaly detection performance of equal-sized representations for each symbolic reprsentation. We utilize KNN + each symbolic representation for anomaly detector, and present the visualization of (i) the raw time series + anomaly labels and (ii) anomaly score generated by each symbolic method.
...
* The TSB-UAD Archive, which contains 18 datasets with about 2000 anomalous time series.
    * Download all 18 datasets [here](https://github.com/TheDatumOrg/TSB-UAD?tab=readme-ov-file).
